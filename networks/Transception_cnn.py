...
class MiT_3_ResInception_cnn(nn.Module):
...
class MyDecoderLayer(nn.Module):
    def __init__(self, input_size, in_out_chan, head_count, token_mlp_mode, n_class=9,
        ...
        if not is_last:
            ...
            # transformer decoder
            self.layer_up = FinalPatchExpand_X4(input_resolution=input_size, dim=out_dim, dim_scale=2, norm_layer=norm_layer)
...
class Transception(nn.Module):
    def __init__(self, num_classes=9, head_count=1, dil_conv=1, token_mlp_mode="mix_skip", inception="135"):
        ...
        self.backbone = MiT_3_ResInception_cnn(image_size=224, in_dim=dims, key_dim=key_dim, value_dim=value_dim, layers=layers,
