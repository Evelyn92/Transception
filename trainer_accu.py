...
def trainer_synapse(args, model, snapshot_path):
    ...
    print(f"The model: {args.model_name}")
    logging.info(f"The model: {args.model_name}")
    ...
    for epoch_num in iterator:
        ...
        # Test
        eval_interval_initial = 20
        eval_interval = args.eval_interval 
        if epoch_num >= int(max_epoch / 2) and epoch_num < int(max_epoch-100) and (epoch_num + 1) % eval_interval_initial == 0:
        ...
        if epoch_num >= int(max_epoch-100) and (epoch_num + 1) % eval_interval == 0:
            filename = f'{args.model_name}_epoch_{epoch_num}.pth'
            save_mode_path = os.path.join(snapshot_path, filename)
            torch.save(model.state_dict(), save_mode_path)
            logging.info("save model to {}".format(save_mode_path))
